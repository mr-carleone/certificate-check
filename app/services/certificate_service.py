...
class CertificateService:
        ...
        try:
            ...
            with socket.create_connection((hostname, 443)) as sock:
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    ).days < settings.CERTIFICATE_EXPIRY_DAYS

                    return CertificateInfo(
                        url=url, valid_until=exp_date, needs_update=needs_update, error=None
                    )
        except Exception as e:
            logger.error(f"Error checking certificate for {url}: {str(e)}")
            return CertificateInfo(url=url, valid_until=None, needs_update=True, error=str(e))
    ...
    def check_host_certificates(host: str) -> HostInfo:
        ...
        return HostInfo(hostname=host, certificates=[cert_info])
